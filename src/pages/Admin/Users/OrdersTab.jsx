									...
									paginatedData.orders.map((order, index) => (
										<tr
											...
											<td className="px-6 py-4">
												{order.orders.reduce((acc, _order) => acc + _order.items.length, 0)}
											</td>
											<td className="px-6 py-4 w-1/6">{format.formatMoney(order?.bill)}</td>
											<td className="px-6 py-4">{format.formatDate(order?.orderedAt)}</td>
											<td className="px-6 py-4">{order?.payment.method}</td>
											<td className="px-6 py-4">
												<span className={createStatusBadge(order?.isCompleted ? "Completed" : "Progressing")}>
