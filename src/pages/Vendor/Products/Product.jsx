...
const Product = ({
  ...
  const validateProduct = () => {
    ...
    if (product.image && product.file.size > 10 * 1024 * 1024) {
      ...
      if (changedSubImages.length > 0 || updatedMainImage) {
        ...
        if (changedSubImages.length > 0) {
          changedSubImages.forEach(index => {
            ...
            subImages[index] = foundImage[product?.subImages[index]?.file?.name]
          })
        }
        subImages = subImages.filter(subImage => typeof subImage === 'string')
        productBody = {
          ...productBody,
          image: mainImage,
          subImages
