...
const Product = ({
  ...
  const [selectedSubSubCategory, setSelectedSubSubCategory] = useState(
    selectedProduct?.subSubCategory || null
  );
  const errorsBody = {
    name: null,
    gender: null,
  ...
  //     category =>
  //       category.id === product.category || category.id === product.category?.id
  //   )
  // }, [categories, product])
  ...
  const ImageCompressor = async (imageFile) => {
    ...
    return new File([compressedImageFile], imageFile.name, {
      type: compressedImageFile.type,
    });
  };
  ...
  const addProduct = async () => {
    try {
      ...
      let compressedImage = await ImageCompressor(product.file);
      formData.append("file", compressedImage);

      for (let index = 0; index < product.subImages.length; index++) {
        if (product.subImages[index].file) {
          let compressedImage = await ImageCompressor(
            product.subImages[index].file
          );
          formData.append("file", compressedImage);
        }
      }
  ...
  const handleSelectSubSubCategory = (e) => {
    ...
    if (
      !subSubCategoryId &&
      !categoryTree[selectedCategory.id][selectedSubCategory.id][
        subSubCategoryId
      ]
    ) {
    ...
    const _selectedSubSubCategory = {
      name: categoryTree[selectedCategory.id][selectedSubCategory.id][
        subSubCategoryId
      ].name,
      id: subSubCategoryId,
    };
                  ...
                  <div className="p-4 border border-gray-200 rounded-lg flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      >
                        ...
                        {selectedCategory &&
                          categoryTree[selectedCategory.id] &&
                          Object.keys(categoryTree[selectedCategory.id]).map(
                            (id) => {
                              if (id === "name") return;
                              return (
                                <option key={id} value={id}>
