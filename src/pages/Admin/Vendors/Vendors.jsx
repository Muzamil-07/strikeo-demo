	...
	const getVendors = async (page) => {
		try {
			const res = await http.get(
				"/vendor?page=" + (page ? page : "1") + (searchedVendor ? "&search=" + searchedVendor : "")
			);
					...
					<div className="flex-grow px-5 md:px-16 py-4">
						{isLoading || !paginatedData.vendors ? (
							...
							<div className="py-4">
								...
								<Stepper setView={setView} getVendors={getVendors} />
							</div>
						) : view === "single-vendor" ? (
							<div className="py-4">
								<div className="flex items-center justify-between">
									<div className="flex items-center gap-4">
										<HiArrowNarrowLeft
											...
											onClick={() => updateView("list")}
										/>
										{`${selectedVendor?.firstName} ${selectedVendor?.lastName} (${selectedVendor?.company.name})`}
									</div>
								</div>
								<VendorTabs
